...
import { JiraTaskAgent } from './task-agent';
...
function parseArgs(): { description?: string; inputFile?: string; epicKey?: string } {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (args[i] === '--epic' && args[i + 1]) epicKey = args[++i];
  }

  return { description, inputFile, epicKey };
}
...
async function promptInteractive(): Promise<string> {
  ...
  return new Promise((resolve) => {
    rl.question('\nDescreva a atividade (o que precisa ser feito): ', (answer) => {
      rl.close();
      resolve(answer.trim());
    });
  });
...
async function main(): Promise<void> {
  ...
  const { description: argDescription, inputFile, epicKey } = parseArgs();

  let taskDescription: string | undefined = argDescription;

  if (!taskDescription && inputFile) {
    const fp = path.resolve(inputFile);
    if (!fs.existsSync(fp)) {
      process.stderr.write(`Error: Input file not found: ${fp}\n`);
      process.exit(1);
    }
    taskDescription = fs.readFileSync(fp, 'utf-8').trim();
  }

  if (!taskDescription) taskDescription = await readStdin();
  if (!taskDescription) taskDescription = await promptInteractive();
  ...
  if (epicKey) process.stderr.write(`  epic=${epicKey}`);
  process.stderr.write('\n' + '─'.repeat(60) + '\n');

  const agent = new JiraTaskAgent(apiKey, jiraConfig);

  try {
    await agent.run(taskDescription, epicKey);
  } catch (err) {
